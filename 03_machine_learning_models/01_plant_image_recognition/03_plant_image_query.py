...
# Note: This script implements plant image similarity search using CLIP model
# It performs Test-Time Augmentation (TTA) to improve query robustness
# The script loads pre-computed embeddings and finds most similar plant images

# Key components:
# - EXIF correction for proper image orientation
# - TTA pipeline with geometric and color transformations  
...
# Usage: python plant_image_query.py --image <path> --k <top_k> --json
...
def exif_correct(img: Image.Image) -> Image.Image:
    """Correct orientation and convert to RGB format"""
...
def tta_pipeline():
    """Lightweight Test-Time Augmentation pipeline"""
...
def main():
    ...
    ap.add_argument("--index", type=str, default="index/embeddings_fp16.npz", help="Index file generated during database building")
    ap.add_argument("--meta", type=str, default="index/meta.json", help="Meta file generated during database building")
    ap.add_argument("--model-local", type=str, default="clip-vit-b32", help="Local directory containing CLIP model")
    ap.add_argument("--fallback-model-id", type=str, default="openai/clip-vit-base-patch32", help="Online model ID to download if local model not found")
    ap.add_argument("--tta", type=int, default=8, help="Number of TTA iterations (including original image)")
    ap.add_argument("--k", type=int, default=10, help="Number of Top-K results to return")
    ...
    # Load model (prefer online model first)
    ...
    try:    
        ...
        print("Online download failed, attempting to load local model")
